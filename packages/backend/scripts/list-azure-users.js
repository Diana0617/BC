...
// Configuración para Azure PostgreSQL desde azure-app-settings.json
const AZURE_DATABASE_URL = 'postgresql://dbadmin:BeautyControl2024!@beautycontrol-db.postgres.database.azure.com:5432/beautycontrol?sslmode=require';

async function listAzureUsers() {
  try {
    console.log('🔐 Conectando a Azure PostgreSQL...\n');

    // Crear conexión a Azure usando la URL completa
    const sequelize = new Sequelize(AZURE_DATABASE_URL, {
      dialect: 'postgres',
      dialectOptions: {
        ssl: {
          require: true,
    ...
    console.log(`📊 Total de usuarios en Azure: ${users.length}\n`);
    console.log('═══════════════════════════════════════════════════════════════════════════════');
