...
const EditTemplateModal = ({ isOpen, onClose, template }) => {
  ...
  useEffect(() => {
    if (template) {
      setFormData({
        ...
        ruleValue: {
          enabled: template.ruleValue?.enabled ?? true,
          description: template.ruleValue?.description || ''
        },
        businessTypes: template.businessTypes || [],
        planTypes: template.planTypes || [],
  ...
  const handleSubmit = async (e) => {
    ...
    const success = await updateTemplate(template.id, formData)
    if (success) {
